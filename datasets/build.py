...
from utils.corruptions import build_corruption
...
def build_dataset(dataset_name, root, corruption, transforms, split):
    if dataset_name == "CUHK-SYSU":
        dataset = CUHKSYSU(root, corruption, transforms, split)
    elif dataset_name == "PRW":
        dataset = PRW(root, corruption, transforms, split)
    return dataset
...
def build_test_loader(cfg):
    ...
    corruption = build_corruption(cfg)
    gallery_set = build_dataset(cfg.INPUT.DATASET, cfg.INPUT.DATA_ROOT, corruption, transforms, "gallery")
    query_set = build_dataset(cfg.INPUT.DATASET, cfg.INPUT.DATA_ROOT, corruption, transforms, "query")
